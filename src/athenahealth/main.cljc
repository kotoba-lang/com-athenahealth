(ns athenahealth.main
  ...
  `Provider`, `Consent` and `PatientAccessRequest` are the exceptions to
  \"resource shapes only\": their `:validate` (and, for
  PatientAccessRequest, `:validate-record`) keys wire real domain format
  checks into the generic create/update handlers -- `Provider` for the NPI
  check digit (45 CFR 162.410), `Consent` for the EU GDPR Art. 9(2)
  special-category-data lawful-basis code (Regulation (EU) 2016/679), and
  `PatientAccessRequest` for the EU EHDS (Regulation (EU) 2025/327)
  Article 3 primary-use access-method enum plus its Article 3(3)
  restriction/reason cross-field rule -- so a provider record with a
  malformed NPI, a consent record citing a lawful-basis code that isn't one
  of the Regulation's ten, or an access request that flags a restriction
  without recording why, is rejected with 400 instead of silently
  persisted."
...
(defn validate-record
  ...
  PatientAccessRequest's restrictionappliedyn=true requiring a non-blank
